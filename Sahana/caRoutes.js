...
/*         // Check if certificate with the same commonName already exists
        const existingCertificate = await Certificate.findOne({ commonName });
        if (existingCertificate) {
            return res.status(400).send('Certificate with this Common Name already exists');
        }    */ 
    ...
    try {
        ...
        // Log the issued certificate details
        console.log('Issued Certificate Details:', {
            username: certificate.username,
            commonName: certificate.commonName,
            issuedBy: certificate.issuedBy,
            certificate: certificate.certificate,
            dateAuthorized: certificate.dateAuthorized,
